...
function generate(defs, seedOverride){
  // Reset the stream so a call is reproducible regardless of what ran
  // before it. Without this, generating the 7-letter set first shifted the
  // jitter for the 24-letter set, and the CLI and characterisation tool
  // reported different ratios for identical geometry.
  //
  // NOTE: letters consume the stream in key order, so changing WHICH letters
  // are generated changes the draw every letter receives. Two runs over
  // different subsets are therefore different samples of the same geometry,
  // not the same samples filtered — which is why an adoption decision has to
  // be taken across several seeds rather than from one draw.
  seed = seedOverride !== undefined ? seedOverride : SEED;
  const letters = {};
...
// Exported so tools/characterize-alphabet.js can reuse this exact geometry
// instead of duplicating the model — a second copy would drift silently.
module.exports = {buildHand, normalize, generate, measure, LETTERS, WORKING_7, ORDER, SAMPLES, JITTER_CM};

// CLI report only when run directly, so `require()` stays side-effect free.
